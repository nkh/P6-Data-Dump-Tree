...
=item sub get_dump($structure_to_dump, $title?, %options?)
...
=item method  get_dump($structure_to_dump, $title?, %options?)

returns a string containing the dump of the data structure

=item sub get_dumper(%options?)

Returns a dumper with the options applied. It supports options:

=begin nested

=item roles
A list of roles the dumper plays

	get_dumper({ roles => (role_1, role_2, ...) ;
...
=head3 %options<display_perl_address> 
...
More roles will be added in the future, Eg: RenderWithUnicode.
...
Data::Dump::Tree::Role::YouRole and an alias in DDTR::YourRole.

=item Data::Dump::Tree::Role::AsciiGlyphs or DDTR::AsciiGlyphs
Uses ASCII codes rather than ANSI to render the tree.

=item Data::Dump::Tree::Role::UnicodeGlyphs or DDTR::UnicodeGlyphs
Uses Unicode to render the tree. this is hte tightes rendering as only one
character is used to display the tree.

=item Data::Dump::Tree::Role::MatchDetails or DDTR::MatchDetails
Give a bit more details about a Match

=item Data::Dump::Tree::Role::PerlString or DDTR::PerlString
Renders string containing control codes (eg: \n, ANSI, ...) with backslashed
codes and hex values.

=item Data::Dump::Tree::Role::SilentSub or DDTR::SilentSub
